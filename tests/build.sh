...
javac -classpath "$ANDROID_JAR" -source 8 -target 8 -encoding UTF-8 -nowarn -d "$OUT/classes" "$ROOT/tests/DesignSmoke.java"
...
"$AAPT" package -f -M "$ROOT/tests/AndroidManifest.xml" -I /system/framework/framework-res.apk -F "$OUT/unsigned.apk"
(cd "$OUT/dex" && "$AAPT" add "$OUT/unsigned.apk" classes.dex)
"$ZIPALIGN" -f -p 4 "$OUT/unsigned.apk" "$OUT/aligned.apk"
apksigner sign --ks "$ROOT/keystore/note-watermark.keystore" --ks-pass pass:notewm123 --key-pass pass:notewm123 --out "$OUT/DesignTests.apk" "$OUT/aligned.apk"
